...
- Upcasts, i.e., casts from a type to a [supertype](#subtyping-and-least-upper-bounds)
  ...
  never result in run-time errors, and, except in the case where the type and
...
- Any casts from `Field` or any `Uint` type to `Boolean` is allowed, cannot
  result in a run-time error, and requires a simple run-time conversion (of `0` to
...
- Any cast from `Bytes<`*m*`>`, *m* != 0, to `Field` or any `Uint` type is allowed,
  ...
  A run-time error occurs if the result exceeds the maximum value of the `Field`
  or `Uint` type.
...
- Any cast from `Bytes<`*m*`>` to `Vector<`*m*, `Field>` or
  ...
  cause a run-time error, but does require a run-time conversion.
- Any cast from `Bytes<`*m*`>` to tuple type `[`*T<sub>1</sub>*, ⋯, *T<sub>m</sub>*`]`,
  ...
  cannot cause a run-time error, but does require a run-time conversion.
- Any cast from `Vector<`*m*, `Uint<0..`*k*`>>`, *k* ≤ 256, to `Bytes<`*m*`>`
  is allowed, cannot cause a run-time error, but does require a run-time conversion.
- Any cast from tuple type `[`*T<sub>1</sub>*, ⋯, *T<sub>m</sub>*`]` to
  `Bytes<`*m*`>`, where each *T<sub>i</sub>* is `Uint<0..`*k*`>` for some
  *k* ≤ 256 is allowed, cannot cause a run-time error, but does require a
  run-time conversion.
