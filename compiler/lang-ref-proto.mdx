...
that the ambiguity is resolved by associating an *else* part with the closest
...
at the same level while requiring the right operand to be at a higher level.
...
Default is not defined for contract types and `Kernel` type (which is defined
...
corresponding relational operation.
