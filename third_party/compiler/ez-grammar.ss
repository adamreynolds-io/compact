          ...
          (define (render-clause clause)
            (if (terminal-clause? clause)
                ...
                (let ([nonterm (format "~a" (syntax->datum (clause-id clause)))]
                      [alias* (map format-alias (nonterminal-aliases clause))])
                  (<h4> ()
                    (<a> ([name nonterm])
                    ...
                    (let ([primary-alias (car alias*)] [secondary-alias* (cdr alias*)])
                      (let loop ([prod* (nonterminal-clause-prod* clause)] [first? #t])
                        (unless (null? prod*)
                          (let ([prod (car prod*)])
                            (<tr> ()
                              (<td> () (when first? (display-string primary-alias)))
                      ...
                      (for-each
                        (lambda (secondary-alias)
                          (<tr> ()
                            (<td> () (display-string secondary-alias))
                            (<td> () (display-string "&xrarr;"))
                            (<td> () (display-string primary-alias))))
                        secondary-alias*)))
                ...
                (define (record anchor name*)
                  (for-each
                    (lambda (name)
                      (let ([a (hashtable-cell env name #f)])
                        (if (cdr a)
                            (warningf 'create-snippets "multiple anchor locations ~s and ~s for anchor-name ~s"
                                      (cdr a) anchor name)
                            (set-cdr! a anchor))))
                    name*))
                (syntax-case req ()
                              ...
                              (hashtable-set! nonterminal-snippets secondary-alias
                                (lambda ()
                                  (<tr> ()
                                    ...
                                    (<td> () (display-string "&xrarr;"))
                                    (<td> () (<a> ([href (format "#~a" (lookup primary-alias env))])
                                               (render-alias primary-alias)))))))
                            secondary-alias*))))
                    (section-clause* section)))
                (grammar-section* grammar))
              (snippets
